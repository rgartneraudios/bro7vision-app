// /data/puffo/economia_lunar.js
export const economia_lunar = {
  keyword: "economia_lunar",
  puente: "¡Okey! Esto tiene mucha tela. BRO7VISION sincronizado con la luna. Dime, ¿lo sabías?",
  data: `
Ajá. El sistema está alineado con las fases reales de la luna. Jaguar te puede dar contexto sideral.
La idea es esta: con cada fase aparecen Vales de descuento entre el 5% y el 15%,
que consigues canjeando Puntos Génesis. Esos Génesis los ganas jugando o viendo BroStories.
Y luego están los Packs lunares, para comprar Halos de Luz, Ecos que son los comentarios de pago,
y Zaps para promocionar canales. Interesante...
El desglose completo de precios y condiciones está en el apartado legal.
  `,
  continua: "Fíjate, si tuviéramos que sacar un titular: cuanto más llena la luna, más descuento. ¿Te queda claro cómo funciona?",
  salida: "MC"
};
export default economia_lunar;
