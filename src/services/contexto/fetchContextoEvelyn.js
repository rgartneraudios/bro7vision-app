// src/services/contexto/fetchContextoEvelyn.js
...
export async function fetchContextoEvelyn(ciudadCodigo = null) {
  try {
    const { data, error } = await supabase
      .from('personaje_update')
      .select(`
        vivencia_actual, estado_animo,
        promo_ciudad, promo_regional, promo_gran_regional,
        promo_metropolis, promo_nacional, promo_mundial,
        special_texto, special_codigo, special_stock, special_activo
      `)
      .eq('personaje_id', 'evelyn')
      .single();
    ...
    return {
      ...
      special: data.special_activo && (data.special_stock ?? 0) > 0 ? {
        texto: data.special_texto, codigo: data.special_codigo, stock: data.special_stock,
      } : null,
    };
  } catch { return null; }
}
