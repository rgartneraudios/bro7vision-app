        ...
        <div style={{ position:'relative', zIndex:2, flex:1, minHeight:0,
          ...
          {loading ? (
            <div style={{ display:'flex', gap:8 }}>
              {[0,1,2].map(i => <div key={i} className="im-dot" style={{ animationDelay:`${i*0.2}s` }} />)}
            </div>
          ) : mensaje ? (
            <p className="im-msg">{mensaje}</p>
          ) : (
            <p className="im-msg" style={{ opacity:0.45 }}>¿Qué cita necesitas? Cuéntame y lo gestionamos.</p>
          )}
