...
const IsabellaCierre = ({
  ...
  const [delivery, setDelivery]   = useState(deliveryProp || 'pickup');
  const [mensajeLocal, setMensajeLocal] = useState(null);

  // IA hook
  const { mensaje: iaMensaje, loading: iaLoading, enviar: enviarIA, bolas: iaBolas } = useAgentIsabellaCierre({
    personaje,
    iaMode,
    isAdmin,
    onHandoff,
  ...
  // Internal precios calculation
  const precios = useMemo(() => ({
    total_final: carrito.reduce((s, i) => s + (i.item_precio_base || 0) * i.qty, 0),
  }), [carrito]);
  ...
  const handleSendConIA = async (texto) => {
    ...
    const accion = await enviarIA(texto, {
      ...
      catalogo,
      comercio,
      perfilUsuario: { nombre: usuario_nombre },
    });
    ...
    switch (accion.tipo) {
      case 'AÑADIR_ITEM': {
        const item = catalogo.find(i => i.id === accion.item_id || i.item_codigo === accion.item_codigo);
        if (!item) break;
        setCarrito(prev => {
          const existing = prev.find(i => i.id === item.id);
          if (existing) return prev.map(i => i.id === item.id ? { ...i, qty: i.qty + (accion.qty || 1) } : i);
          return [...prev, { id: item.id, item_nombre: item.nombre, item_precio_base: item.precio_base, qty: accion.qty || 1, tipo: 'servicio' }];
        });
        break;
      }
      case 'CAMBIAR_CANTIDAD': {
        setCarrito(prev => prev.map(i => i.id === accion.item_id ? { ...i, qty: Math.max(1, i.qty + (accion.delta ?? 0)) } : i));
        break;
      }
      case 'RESTAR_ITEM': {
        setCarrito(prev => prev.map(i => i.id === accion.item_id ? { ...i, qty: Math.max(1, i.qty - 1) } : i));
        break;
      }
      case 'QUITAR_ITEM': {
        setCarrito(prev => prev.filter(i => i.id !== accion.item_id));
        break;
      }
      case 'ACTIVAR_VALE':
      case 'CAMBIAR_VALE': {
  ...
  const handleSend = () => { if (!input.trim() || loading) return; handleSendConIA(input.trim()); };
