...
const SOLOO169_STYLES = `
  @keyframes visor-glow-blue {
    0%,100% { box-shadow: 0 0 20px rgba(0,0,154,0.5), 0 0 60px rgba(0,0,154,0.2), inset 0 0 20px rgba(0,0,154,0.05); }
    50%   { box-shadow: 0 0 40px rgba(0,0,154,0.8), 0 0 100px rgba(0,0,154,0.3), inset 0 0 30px rgba(0,0,154,0.10); }
  }
  .soloo169-visor {
    border: 2px solid rgba(0,0,154,0.4);
...
.orbita-cabeza {
  ...
  border-radius: 50%;
  box-shadow: 0 0 10px 2px #00ffff, 0 0 20px #00ffff;
}
...
const SoloO169 = ({ videoUsers, balances, setBalances, session, realityMode, onOpenProfile, selectedForestUser, savedUserIndex, setShow169 }) => {
  ...
  useEffect(() => {
    let active = true;
    resolveVideoFromCandidates(getVideoCandidates(4, getMoonSuffix(), getTurno(), 0, null))
...
<div className="pointer-events-none">
  ...
  style={{ borderRadius: '1.5rem', border: '2px solid rgba(0,0,154,0.4)', boxShadow: '0 0 20px rgba(0,0,154,0.5), 0 0 60px rgba(0,0,154,0.2)' }}>
     ...
     {/* ══ FOOTER PC ══ */}
      ...
      <div className="md:hidden absolute top-[48%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-[20]"
        ...
        <div className="relative w-[62vw] aspect-[9/19] flex flex-col items-center">
          <div className="relative w-full h-full overflow-hidden bg-black soloo169-visor">
