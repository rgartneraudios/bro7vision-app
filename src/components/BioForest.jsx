...
const BioForest = ({ videoUsers, balances, setBalances, session, realityMode }) => {
  ...
  // --- 1. IDENTIDAD (Definida primero para evitar ReferenceError) ---
  ...
  // --- 2. CONFIGURACIÓN DE REALIDAD (Incluye Summer Reef) ---
  const config = useMemo(() => {
    switch(realityMode) {
      case 'blackhole': return { 
          video: '/videos/eclipse_mode.mp4', colors: ['text-[#FFD700]', 'text-orange-400'], 
          font: 'font-serif italic', border: 'border-[#C7AF38]/40 shadow-[0_0_40px_rgba(199,175,56,0.2)]', 
          reactionColor: 'orange', labelClass: 'text-orange-500', labelText: 'ECLIPSE ZENITH', navColor: 'text-yellow-500' 
      };
      case 'winter': return { 
          video: '/videos/winter_mode.mp4', colors: ['text-orange-400', 'text-yellow-200'], 
          font: 'font-black', border: 'border-orange-900/40 shadow-[0_0_30px_rgba(251,146,60,0.1)]', 
          reactionColor: 'orange', labelClass: 'text-orange-400', labelText: 'WINTER CABIN', navColor: 'text-orange-500' 
      };
      case 'summer': return { 
          video: '/videos/summer_mode.mp4', colors: ['text-cyan-300', 'text-blue-400', 'text-emerald-200'], 
          font: 'font-black tracking-tighter', border: 'border-cyan-400/30 shadow-[0_0_30px_rgba(34,211,238,0.2)]', 
          reactionColor: 'cyan', labelClass: 'text-cyan-400', labelText: 'SUMMER REEF', navColor: 'text-cyan-300' 
      };
      default: return { 
          video: '/videos/bio_landing.mp4', colors: ['text-cyan-400', 'text-fuchsia-400'], 
          font: 'font-black', border: 'border-white/10 shadow-[0_0_60px_rgba(0,0,0,1)]', 
          reactionColor: 'cyan', labelClass: 'text-cyan-400', labelText: 'GÉNESIS FOREST', navColor: 'text-cyan-400' 
      };
    }
  }, [realityMode]);
  ...
  const handlePlayTribe = () => {
    ...
    if (!eco) return;
    window.speechSynthesis.cancel(); audioTribeRef.current.pause();
    if (eco.audio_link) {
    ...
    } else if (eco.text?.startsWith('[TTS]')) {
        const cleanText = eco.text.replace('[TTS]', '');
  ...
  const handleAction = async (type) => {
    ...
    // TRUCO PARA EL NICK: Intentamos sacar el alias más actualizado
    const myAlias = session?.user?.user_metadata?.alias || session?.user?.email?.split('@')[0] || 'ANÓNIMO';
  ...
  useEffect(() => {
    ...
    if (isUUID) {
        ...
        .then(({data}) => {
            if (data) {
                setVisualEchos(data.filter(e => !e.audio_link && !e.text?.startsWith('[TTS]')));
                setAudioPool(data.filter(e => e.audio_link || e.text?.startsWith('[TTS]')));
            }
        });
    } else {
  ...
  const forestStyles = `
    @keyframes spiritFade {
        ...
        10% { opacity: 1; transform: translateY(0); filter: blur(0px); }
        85% { opacity: 1; }
        100% { opacity: 0; transform: translateY(-30px); filter: blur(15px); }
    }
    .animate-spirit { animation: spiritFade 8s ease-in-out forwards; }
    @keyframes glowSwim { 
  ...
  return (
    <div className="absolute inset-0 z-0 bg-black overflow-hidden select-none font-mono">
      ...
      {config.video && <video src={config.video} autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover opacity-40 z-[1]" />}
