...
const detectarSalidaNova = (texto) => {
  const t = norm(texto);
  const quiereSalir = KEYWORDS_SALIDA_NOVA.some(kw => t.includes(norm(kw)));
  if (!quiereSalir) return null;
  return {
    salida:  true,
    mensaje: FRASES_SALIDA_NOVA[Math.floor(Math.random() * FRASES_SALIDA_NOVA.length)],
  };
};
