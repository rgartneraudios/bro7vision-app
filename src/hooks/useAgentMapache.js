...
function mapacheBot({ textoUser = '', intencion = null, entidad = null, hayTarjetas = false, update = null }) {
  ...
  if (entidad) {
    if (intent === 'play') return { handoff: 'AUDIO_PLAY', codigo: entidad.bro_id, mensaje: elegir(MP_PLAY), bolas: [] };
    return { handoff: false, mensaje: `${elegir(MP_DESCRIPCION)} ${entidad.nombre || entidad.bro_id} — ${entidad.description || entidad.categoria || 'sin descripción disponible'}.`, bolas: [] };
  }
...
function amiBot({ textoUser = '', intencion = null, entidad = null, hayTarjetas = false, update = null }) {
  ...
  if (entidad) {
    if (intent === 'play') return { handoff: 'AUDIO_PLAY', codigo: entidad.bro_id, mensaje: elegir(AMI_PLAY), bolas: [] };
    return { handoff: false, mensaje: `${elegir(AMI_DESCRIPCION)} ${entidad.nombre || entidad.bro_id} — ${entidad.description || entidad.categoria || 'sin descripción disponible'}.`, bolas: [] };
  }
